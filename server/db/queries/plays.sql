...
ON CONFLICT(host_name, starts_at, sport, COALESCE(venue_id, 0))
...
WHERE p.ends_at > strftime('%Y-%m-%d %H:%M:%S+00:00', 'now')
  ...
  AND (sqlc.narg('source') IS NULL OR p.source = sqlc.narg('source'))
  AND (sqlc.narg('availability') IS NULL OR p.slots_left > 0)
